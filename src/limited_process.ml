...
module Make (PI : Process_info.S) : S = struct
  ...
  let check_limits pid memory time =
    match PI.of_pid pid with
    | Some info -> (
        let passed_memory =
          Option.value_map memory ~default:`Ok ~f:(fun limit ->
              if Int64.(info.PI.memory_size <= of_int limit) then `Ok
              else `Memory)
        in
        match passed_memory with
        | `Ok ->
            let passed_time =
              Option.value_map time ~default:`Ok ~f:(fun limit ->
                  if Time_ns.(Span.(diff (now ()) !start_time <= limit)) then
                    `Ok
                  else `Runtime)
            in
            passed_time
    ...
    | `In_the_parent child_pid ->
        ...
        let result =
          ...
          | `Saved ->
              {
                result with
                stdout_fn = Some stdout_fn;
                stderr_fn = Some stderr_fn;
              }
          | _ -> result
        in
