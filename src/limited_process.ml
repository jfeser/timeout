...
module Make (PI : Process_info.S) : S = struct
  ...
  let check_limits (pid : Pid.t) (memory : int option)
      (time : Time_ns.Span.t option) : [`Ok | `Memory | `Runtime] =
    match PI.of_pid pid with
    | Some info ->
        let passed_memory =
          Option.value_map memory ~default:`Ok ~f:(fun limit ->
              if info.PI.memory_size <= Int64.of_int limit then `Ok
              else `Memory )
        in
        if passed_memory = `Ok then
          let passed_time =
            Option.value_map time ~default:`Ok ~f:(fun limit ->
                if Time_ns.diff (Time_ns.now ()) !start_time <= limit then `Ok
                else `Runtime )
          in
          passed_time
        else passed_memory
