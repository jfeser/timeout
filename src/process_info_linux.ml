...
let seconds_of_jiffies js =
  ...
  Float.(Int64.to_float js_int / jps) |> Time_ns.Span.of_sec

let of_pid pid =
  Option.map (Procfs.with_pid pid) ~f:(fun proc ->
      { user_time=
          seconds_of_jiffies proc.Procfs.Process.stat.Procfs.Process.Stat.utime
      ; system_time=
          seconds_of_jiffies proc.Procfs.Process.stat.Procfs.Process.Stat.stime
      ; memory_size=
          (let rss =
